...
pipeline {
  ...
  stages {
      ...
      steps {
        script {
          ...
          serviceDirs.each { service ->
            // changedFiles라는 리스트를 조회해서 service 변수에 드러온 서비스 이름과
          ...
          if (env.CHANGED_SERVICES == "") {
            echo "No changes detected in service directories. Skipping build and"
            // 성공 상태로 파이프라인을 종료
    ...
    stage('Build Changed Services') {
      ...
      // 이전 스테이지에서 세팅한 CHANGED_SERVICES 라는 환경변수가 비어있지 않아야만 실행.
      when {
